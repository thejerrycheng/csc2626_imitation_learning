...
class PID(object):
    ...
    def compute_error_derivative(self):
        if len(self.bounded_window_of_errors) < 2:
            # Not enough data points to calculate derivative
            return 0.0

        dt1 = self.timestamps_of_errors[-1] - self.timestamps_of_errors[-2]
        if dt1 <= 0:
            print(f"Warning: Non-positive dt1 value encountered: {dt1}. Skipping derivative calculation.")
            return 0.0  # Return zero if there's an error in the time difference
    ...
    def compute_error_integral(self):
        if len(self.timestamps_of_errors) < 2:
            return 0.0
        
        dt1 = self.timestamps_of_errors[-1] - self.timestamps_of_errors[-2]
        if dt1 <= 0:
            print(f"Warning: Non-positive dt1 value encountered while integrating: {dt1}. Skipping integration.")
            return 0.0
    ...
    def update(self, error, timestamp):
        if len(self.timestamps_of_errors) > 0 and timestamp <= self.timestamps_of_errors[-1]:
            print(f"Warning: Non-incremental timestamp encountered: {timestamp}. Skipping this update.")
            return  # Ignore updates if the timestamp is not strictly increasing
