...
class FullStateCarRacingEnv(CarRacing):
    ...
    def __init__(self, seed=0):
        ...
        self.distance_pid = PID(
            kp=0.25 / 2, ki=0, deriv_prediction_dt=100.0 / FPS, max_deriv_noise_gain=60, max_window_size=20
        )
    ...
    def get_cross_track_error(self, car, track):
        ...
        pld_min = np.finfo(np.float32).max
        dest_min = 0
        ...
        R_world_trackframe = np.array(
            [[cos(target_heading), sin(target_heading)], [-sin(target_heading), cos(target_heading)]]
        )
        ...
        cross_track_error = p_car_trackframe[0][0]

        # print (error_heading * 180.0 / 3.14, cross_track_error, p_car_trackframe[1][0])
        return error_heading, cross_track_error, dest_min

    def get_expert_action(self, car, track):
        current_speed = sqrt(self.car.hull.linearVelocity.x**2 + self.car.hull.linearVelocity.y**2)
        eh, cte, _ = self.get_cross_track_error(car, track)

        es = self.target_speed - current_speed

        self.orientation_pid.update(eh, self.t)
        self.distance_pid.update(cte, self.t)
    ...
    def step(self, action):
        ...
        expert_action, eh, cte = self.get_expert_action(self.car, self.track)
        if abs(cte) > 10:
            print("Learner deviated too far!")
            done = True

        return self.state, expert_action, step_reward, done, {"cross_track_error": abs(cte)}
